...
const loadLatestMessages = async (roomId, limit = 20, beforeDate = new Date(), dbName) => {
    const StudentsMessage = await getStudentsMessageSchema(dbName);
...
const updateNotSeenMessagesToSeen = async (userId, chattingWith, now, dbName) => {
    const StudentsMessage = await getStudentsMessageSchema(dbName);
    return await StudentsMessage.updateMany(
        ...
        { $set: { status: 'vu', seenAt: now } } // update
    );
};

const updateSeenAtDate = async (messageId, date, dbName) => {
    const StudentsMessage = await getStudentsMessageSchema(dbName);
    return await StudentsMessage.findByIdAndUpdate(messageId, {
        seenAt: date
    },
        { new: true })
};
