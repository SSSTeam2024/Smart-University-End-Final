...
const createStudentsRoom = async (req, res) => {
    try {
        const { nbr_unreaded_msg, last_message, students } =
            req.body;
...
const getRoomsByStudentId = async (req, res) => {
    try {
        ...
        const rooms = await StudentsRoomService.getRoomsByStudentId(studentId, useNewDb(req));
        res.json(rooms);
    } catch (err) {
...
const updateLastMessage = async (req, res) => {
    try {
        ...
        const updatedRoom = await StudentsRoomService.updateLastMessage(roomId, messageId, useNewDb(req));
...
const getRoomByParticipants = async (req, res) => {
    try {
        ...
        const room = await StudentsRoomService.getRoomByParticipants(senderId, receiverId, useNewDb(req));

        if (!room) {
            return res.status(404).json({ message: "Aucune room trouvée entre ces deux étudiants." });
        }
