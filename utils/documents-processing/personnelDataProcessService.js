...
async function processFrenshPersonnelData(demandId) {

    const demandData = await demandPersonnelDao.getDemandePersonnelById(demandId)
    console.log("demandData", demandData)
    const personnel = demandData.personnelId;

    const allGlobalVars = await globalVarsDao.getVariableGlobales();
    ...
    const generatedDocs = await generatedDocDao.getGenerartedDocsByModelId(demandData.piece_demande._id);
    let orderNumber = '';
    ...
    const qrImage = await fetchQRImageBuffer('http://verify.eniga.smartschools.tn/verify.html?id=' + String(hashedData));
    ...
    const data = {
        ...
        link: `http://verify.eniga.smartschools.tn/verify.html?id=${String(hashedData)}`
    }
    return {
        information: data,
        generatedDocInfo: {
...
async function processArabicPersonnelData(demandId) {
